...
SKIP_PATTERNS: tuple[str, ...] = (
    ...
    # IDs internos Amazon (podem aparecer em qualquer coluna)
    "#1.value", "[marketplace_id=", "amzn1.volt",
    "contribution_sku", "contribution_goods",
)
...
class AmazonSheetReader:
    ...
    def _find_data_start(df_raw: pd.DataFrame, header_idx: int, max_scan: int = 8) -> int:
        ...
        for i in range(header_idx + 1, header_idx + 1 + max_scan):
            ...
            # Verifica TODAS as células — IDs internos podem estar em colunas não-A
            all_vals = [v.strip() for v in row.values]
            # Pula se qualquer célula contém marcadores de IDs internos Amazon
            if any('#' in v or '[' in v for v in all_vals):
                continue
            if any(p in v.lower() for v in all_vals for p in SKIP_PATTERNS):
                continue
            # Pula linha de exemplo padrão da Amazon
            if any(v.upper() in ("ABC123", "EXAMPLE", "EXEMPLO") for v in all_vals):
                continue
            # Pula metadados longos
            if any(len(v) > 200 for v in all_vals):
                continue
            return i
    ...
    def read(self, file_like: io.BytesIO | str) -> AmazonReadResult:
        ...
        if sheet_name is None:
            return AmazonReadResult(
                ...
                total_rows=0,
                valid_rows=0,
                errors=["Aba 'Template' ou 'Modelo' não encontrada no arquivo Amazon."],
            )
    ...
    def _resolve_sheet(self, sheets: list[str], warnings: list[str]) -> Optional[str]:
        # Tenta primeiro os candidatos exatos (Template, Modelo)
        candidates = self._config.get("sheet_candidates", ["Template", "Modelo"])
        for candidate in candidates:
            if candidate in sheets:
                return candidate
        # Fallback: procura aba que contém "template" ou "modelo" (case-insensitive)
