...
MARKETPLACES = ["Selecione o Marketplace", "Magalu", "Mercado Livre", "Shopee", "Temu", "Vendor"]
...
with st.sidebar:
    ...
    amazon_file = st.file_uploader(
        "Planilha Amazon (.xlsx, .xlsm, .xls)", type=["xlsx", "xlsm", "xls"], key="amazon_upload"
    )
    dest_file = st.file_uploader(
        f"Template {marketplace} (.xlsx, .xlsm, .xls)", type=["xlsx", "xlsm", "xls"], key="dest_upload"
    )
    ...
    run_btn = st.button(
        "▶ Processar",
        type="primary",
        disabled=not (amazon_file and dest_file and template_ok),
        ...
        if result.mapping_result:
            ...
            with col_f1:
                filter_strategy = st.multiselect(
                    "Estratégia",
                    ["fixed+synonym", "learned", "similarity", "ai", "unmapped"],
                    default=["fixed+synonym", "learned", "similarity", "ai", "unmapped"],
